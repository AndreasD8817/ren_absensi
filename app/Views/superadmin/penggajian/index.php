    ...
    <main class="flex-1 overflow-y-auto p-8">
        ...
        <div class="grid grid-cols-3 gap-6 mb-8">
            <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 border-l-4 border-l-blue-500">
                ...
                <h3 class="text-3xl font-bold text-gray-800"><?= count($ringkasan) ?></h3>
            ...
            <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 border-l-4 border-l-red-400">
                ...
                <h3 class="text-2xl font-bold text-gray-800">Rp <?= number_format($total_potongan_semua, 0, ',', '.') ?></h3>
            ...
            <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 border-l-4 border-l-green-500">
                ...
                <h3 class="text-2xl font-bold text-gray-800">Rp <?= number_format($total_gaji_bersih_semua, 0, ',', '.') ?></h3>
        ...
        <div class="flex justify-between items-center mb-5">
            <div class="flex gap-2">
                ...
                <a href="<?= BASE_URL ?>/superadmin/export_gaji?bulan=<?= $bulan ?>&tahun=<?= $tahun ?>&format=pdf" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-red-100 text-red-700 text-sm font-bold rounded-xl hover:bg-red-200 transition-colors">
                ...
                <table class="w-full text-sm">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-semibold text-gray-800"><?= htmlspecialchars($g['nama_lengkap']) ?></p>
                                        <p class="text-gray-400 text-xs"><?= htmlspecialchars($g['nip']) ?></p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-gray-600"><?= htmlspecialchars($g['nama_cabang']) ?></td>
                            <td class="px-6 py-4 text-right text-gray-700">Rp <?= number_format($g['nilai_gaji_pokok'],0,',','.') ?></td>
                            <td class="px-6 py-4 text-right text-red-500 font-semibold">- Rp <?= number_format($g['total_potongan_telat_alfa'],0,',','.') ?></td>
                    ...
                    <tfoot class="bg-gray-50 border-t-2 border-gray-200">
                        <tr>
                            ...
                            <td class="px-6 py-4 text-right font-bold text-gray-700">Rp <?= number_format(array_sum(array_column($data_gaji,'nilai_gaji_pokok')),0,',','.') ?></td>
                            <td class="px-6 py-4 text-right font-bold text-red-500">- Rp <?= number_format($total_potongan_semua,0,',','.') ?></td>
                            <?php 
                            ...
                            ?>
                            <td class="px-6 py-4 text-right font-bold text-gray-500">- Rp <?= number_format($total_bpjs_semua,0,',','.') ?></td>
                            <td class="px-6 py-4 text-right font-bold text-green-700 text-base">Rp <?= number_format($total_gaji_bersih_semua,0,',','.') ?></td>
                            <td colspan="2"></td>
                        </tr>
                    </tfoot>
                </table>
...
<div id="modal-slip" class="fixed inset-0 z-50 hidden items-start justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto pt-12 pb-12">
    <div class="bg-white rounded-sm shadow-2xl w-full max-w-4xl font-mono text-sm relative mt-4">
        ...
        <div id="area-cetak-slip" class="p-10 bg-white text-black">
            <!-- Header Perusahaan -->
            <div class="mb-4">
...
<script>
    ...
    function bukaSlipGaji(data) {
        ...
        const sub1 = gapok + tjab + ttrans + tmakan + thadir + overtime + tlain;
    ...
    async function generateGaji() {
        const { isConfirmed } = await Swal.fire({
            ...
            html: 'Sistem akan menghitung otomatis gaji bersih berdasarkan data absensi bulan <b><?= $nama_bulan[$bulan] ?> <?= $tahun ?></b>.',
        ...
        Swal.fire({ title: 'Memproses...', text: 'Menghitung kalkulasi gaji seluruh pegawai.', allowOutsideClick: false, didOpen: () => Swal.showLoading() });

        const resp = await fetch('<?= BASE_URL ?>/superadmin/generate_gaji', {
            method: 'POST',
            body: new URLSearchParams({ bulan: '<?= $bulan ?>', tahun: '<?= $tahun ?>' })
        });
    ...
    async function publishGaji() {
        const { isConfirmed } = await Swal.fire({
            title: 'Publish Gaji?',
            html: 'Data gaji <b><?= $nama_bulan[$bulan] ?> <?= $tahun ?></b> akan dipublikasikan dan bisa dilihat oleh pegawai. Tindakan ini <b>tidak dapat dibatalkan</b>.',
